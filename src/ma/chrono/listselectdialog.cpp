...
namespace ma
{
  namespace chrono
  {
    ListSelectDialog::ListSelectDialog(QWidget* parent, std::auto_ptr<ListPage> listPage)
      : CommandDialog(parent)
    {
      setWindowIcon(listPage->windowIcon());
      setWindowTitle(tr("Select - %1").arg(listPage->windowTitle()));

      setWindowFlags(static_cast<Qt::WindowFlags>(Qt::Dialog | Qt::WindowCloseButtonHint));
      setModal(true);
      resize(600, 400);
      ...
      QObject::connect(listPage_, SIGNAL(currentChanged(const QModelIndex&, const QModelIndex&)), SLOT(on_listPage_currentChanged(const QModelIndex&, const QModelIndex&)));
    ...
    void ListSelectDialog::on_listPage_currentChanged(const QModelIndex& current, const QModelIndex& /*previous*/)
    {
      if (current.isValid())
      {
