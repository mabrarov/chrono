...
const char* insertGeneratorSql = "insert into %1 (%2, %3) values (?, ?)";
const char* loadGeneratorSql   = "select %3 from %1 where %2 = ?";
const char* updateGeneratorSql = "update %1 set %3 = ? where %2 = ? and %3 = ?";
...
Generator::QueryOption::QueryOption(const QString& table, 
    const QString& generatorIdColumn, const QString& lastValueColumn)
  : insertGenerator_(QString::fromLatin1(insertGeneratorSql)
        .arg(table).arg(generatorIdColumn).arg(lastValueColumn))
  , loadLastValue_(QString::fromLatin1(loadGeneratorSql)
        .arg(table).arg(generatorIdColumn).arg(lastValueColumn))
  , updateLastValue_(QString::fromLatin1(updateGeneratorSql)
        .arg(table).arg(generatorIdColumn).arg(lastValueColumn))
{
}
...
Generator::ConcurrentOption::pauseDuration() const
{
  return pauseDuration_;
}
              
Generator::Generator(const QueryOption& queryOption,
...
void Generator::getNextBlock(const QSqlDatabase& connection)
{
  ...
  while (!done)
  {
    ...
    if (lastValue)
    {
      ...
      if (value >= endValue)
      {
        boost::throw_exception(
            std::overflow_error("Generator: maximum value reached"));
      }
      else
      {
        qint64 newValue;
    ...
    {
      boost::throw_exception(
          std::overflow_error("Generator: maximum attemp count reached"));
    }
    else
    { 
...
bool Generator::updateLastValue(const QSqlDatabase& connection, 
    qint64 currentValue, qint64 newValue)
{
  bool done = false;
  QSqlQuery statement(connection);           
